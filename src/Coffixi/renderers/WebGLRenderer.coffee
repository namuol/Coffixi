...
define 'Coffixi/renderers/WebGLRenderer', [
  ...
  ###
  class WebGLRenderGroup extends Module
    ...
    render: (projectionMatrix) ->
      WebGLRenderer.updateTextures @textureFilter
      gl = @gl
    ...
    removeDisplayObject: (displayObject) ->
      ...
      if batchToRemove
        ...
        if index is 0 or index is @batchs.length - 1
          ...
          WebGLRenderer.returnBatch batchToRemove  if batchToRemove instanceof WebGLBatch
          return
        if @batchs[index - 1] instanceof WebGLBatch and @batchs[index + 1] instanceof WebGLBatch
          if @batchs[index - 1].texture is @batchs[index + 1].texture and @batchs[index - 1].blendMode is @batchs[index + 1].blendMode
            ...
            WebGLRenderer.returnBatch batchToRemove  if batchToRemove instanceof WebGLBatch
            WebGLRenderer.returnBatch @batchs[index + 1]
  ...
  ###
  class WebGLRenderer
    @WebGLRenderGroup: WebGLRenderGroup
    @_defaultFrame: new Rectangle(0, 0, 1, 1)
    @Float32Array: Utils.Float32Array
    @Uint16Array: Utils.Uint16Array
    ...
    constructor: (width, height, view, transparent, @textureFilter=BaseTexture.filterModes.LINEAR, @resizeFilter=BaseTexture.filterModes.LINEAR) ->
      ...
      @resize @width, @height
      @contextLost = false
      @stageRenderGroup = new WebGLRenderGroup gl, @textureFilter

    ###
    @private
    ###
    @getBatch: ->
      if WebGLBatch._batchs.length is 0
